...
Matrix2d.prototype.histogram = function(numBins) {
  var min = this.min();
  var max = this.max();
  ...
  this.apply(function(row, column) {
    ...
    var bin = Math.round((value - min)/binSize);
    histogram[bin]++;
  });
  histogram.min = min;
  histogram.max = max;
...
Matrix2d.prototype.threshold = function(threshold, result) {
  if (!defined(result)) {
    result = new Matrix2d(this.columns, this.rows);
  }
  if (result.rows !== this.columns || result.columns !== this.rows) {
  ...
  matrix.apply(function(row, column) {
    result.set(row, column, matrix.get(row, column) >= threshold);
  });
  return matrix;
}
