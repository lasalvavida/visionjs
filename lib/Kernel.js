...
Kernel.laplacian = function(rows, columns) {
  var kernel = new Matrix2d(rows, columns);
  kernel.fill(1);
  var halfRows = Math.floor(rows / 2);
  var halfColumns = Math.floor(columns / 2);
  kernel.set(halfRows, halfColumns, -(rows * columns - 1));
  return kernel;
};

Kernel.laplacianOfGaussian = function(rows, columns, options) {
  options = defaults(options, {
    sigma : 0.85
  });
  var sigma = options.sigma;
  ...
  for (var i = -w; i <= w; i++) {
    for (var j = -h; j <= h; j++) {
      value = (-1/(Math.PI * sigma * sigma * sigma * sigma)) * (1 - (i * i + j * j)/(2 * sigma * sigma)) * Math.exp(-(i * i + j * j)/(2 * sigma * sigma));
      kernel.set(i + w, j + h, value);
      total += value;
    }
  }
